...
# 4x4 Key Layout
key_layout = [
    [f"{chr(65 + row)}{col + 1}" for col in range(4)] \
        for row in range(4)
]
...
def normalize_key_binding(binding):
    ...
    parts = binding.split("+")
    normalized_parts = [mapping.get(part.lower(), part) \
                        for part in parts]
    return "+".join(normalized_parts)


def find_keypad_port(com_ports, expected_message="INIT"):
    # Nested function to check if a given port contains the 
    # expected message
    def check_port(port_info):
         # Debug: Log the port being checked
        print(f"Check port: {port_info.device}") 
        try:
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        ...
        futures = {
            executor.submit(check_port, port): \
                port for port in com_ports
        }
        # Process completed tasks as they finish
        for future in concurrent.futures.as_completed(futures):
            result = future.result()
            if result:  
                # If a port returns a valid device then return
                return result
    return None
...
# Find the Keypad among all ports
...
# This function sets a given module to the currently 
# selected module and updates the gui
def switch_module(module):
    ...
    if (modules):
        # Set the dropdown selection to the current module
        module_var.set(current_module)  
    else:
        module_var.set("No modules")  

    # Update the display
    update_display(is_initializing=False)


# This is the main communication function between this script and 
# the main controller (the hardware) via COM-Port
    ...
    while True:
        try:
            ...
            if serial_connection.in_waiting > 0:
                # Read and decode a line of data from the 
                # serial connection
                message = (
    serial_connection.readline().decode(errors="ignore").strip()
                ...
                if message:
                    ...
                    if message == "START":
                        ...
                        address = None
                        data = None
                        # Reset the pressed keys list
                        pressed_keys = []  

                    # END signal
                    elif message == "END":
                        if debug: print("END")
                        # Process pressed keys if address and 
                        # keys are set
                        if address is not None and pressed_keys:
                            ...
                            if module_name:
                                for key in pressed_keys:
                                    # Check if the key has a 
                                    # mapping for the current module
                                    if key in \
                                        key_mappings[module_name]:
                                        # Saves the mapped key 
                                        # for the pressed Button
                                        press = \
                                    key_mappings[module_name][key]
                                        if press:
                                            if debug: print(
                                                f"Key {key} on \
                                        {module_name} activated.")
                                            # Simulate the press of 
                                            # the mapped key or keys
                                        ...
                                        if debug: print(
                                            f"Invalid key {key} for \
                                                {module_name}.")
                            else:
                                if debug: print(f"Invalid address: \
                                                {address}")
                        else:
                            if debug: print("Error: \
                                            Missing address or keys!")
                    ...
                    # LIST signal to update module information
                        ...
                        new_modules = {}
                        while True:
                            # Read module information until END 
                            # is received
                            module_message = (
                            ...
                            # If the first message is a digit, 
                            # then its the module address
                            elif module_message.isdigit() and \
                                int(module_message) in range(2,8):
                                address = int(module_message)
                                if debug: print(f"{address} \
                                                (Address)")
                                # Read function associated with the 
                                # address
                                function_message = (
                                    serial_connection.readline()
                                    .decode(errors="ignore")
                                 ...
                                 # If the second message is a digit, 
                                 # then its the function number 
                                if function_message.isdigit():
                                    function = int(function_message)
                                    if debug: print(f"{function} \
                                                    (Function)")
                    ...
                    f"Key mappings created for {module_name}.")
                                        ...
                                        print(f"Unknown function \
                                {function} for Address {address}.")

                        # Update current module and module list 
                        # if needed
                        ...
                        # Update the current module only if 
                        # there are changes in the modules
                        if new_modules != modules:
                            ...
                            # Sort the module names and pick the 
                            # first one as the current module
                            if (new_modules):
                             current_module = sorted(
                                new_modules.keys(), reverse=True
                            )[0]
                            ...
                            if debug: print(f"Module count: \
                                            {module_count}")

                    # Handle messages (Data) that are not 
                    # "END" or "INIT"
...
if keypad_port:
    ...
    read_thread = threading.Thread(
        ...
        daemon=True  # Set thread as a daemon to terminate 
...
def update_display(is_initializing=True):
    ...
    if not is_initializing:
        if module_count:
            module_count_label.configure(text=\
                        f"Module count: {module_count}")
        else:
            module_count_label.configure(text=\
                                         "Module count: No modules")
            ...
            if key in loaded_data:  
                for sub_key in key_mappings[key].keys(): 
                    if sub_key in loaded_data[key]: 
                        key_mappings[key][sub_key] = \
                            loaded_data[key][sub_key]
...
def assign_key(row, col):
    ...
    # Normalize the key binding (important for basic keys like 
    # Control or Alt)
...
def reset_key(row, col):
    ...
    # Reset the mapping for the current module's key by 
...
module_dropdown = ctk.CTkOptionMenu(
    ...
    # Function to call when a module is selected
...
for row in range(4):
    for col in range(4):
        ...
        # Create a label to display the key name and binding 
        # (initially empty)
        ...
        labels[row][col].pack(expand=True, fill="both", padx=5, \
                              pady=5)
        ...
        button.grid(row=0, column=0, padx=(2, 1), pady=(2, 0), \
                    sticky="ew")
        ...
        reset_button.grid(row=0, column=1, padx=(1, 2), \
                          pady=(2, 0), sticky="ew")
...
def open_info_window():
    ...
    # Calculate position to open info window 
...
module_count_label.grid(row=0, column=1, sticky="w", \
                        padx=10, pady=10) 
